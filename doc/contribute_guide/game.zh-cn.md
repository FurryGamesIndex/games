...
一般简化描述应短于游戏完整描述，但完整描述本身较短的除外。简化描述的长度最大不得超过 450 个字符（半宽），推荐长度为 200 至 400 个字符（半宽）。
...
缩略图可以选择包含游戏标题字样，但也需清晰可见，不得作为角标存在于图中。裁剪带有标题文字的图片时，不得将文字拦腰截断。

在规格上，缩略图的宽度为 360px，高度为 168px。在不造成明显视觉差异的条件下，文件大小应小于 20KiB，最大不得超过 50KiB。

对于来自 Steam 的缩略图，已自动符合该比例，只需要按比例缩放到 360x168 px 即可。对于来自其他地方的缩略图，通常情况下都将导致丢失信息。应尽量保证主要内容不被裁剪掉。
...
> 若游戏名称之间读音相同，或在不同地区的目标语言都能对应唯一字形的情况下，该游戏非目标语言的名称可以省略，例如 `家有大猫 / 家有大貓` 的游戏名称可仅写为 `家有大猫` 。
