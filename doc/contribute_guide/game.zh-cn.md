...
你也应该查看一些 [`games`](https://github.com/FurryGamesIndex/games/tree/master/games) 目录下已有的数据文件作为参考。

### 数据文件排版

为了便于阅读，下面所说的每一部分直接都要插入一个空行。
...
如果游戏有官方英文，应写英文名称。（和游戏 ID 不同，空格不转换为下划线）。我们会在后面介绍国际化机制，在哪里输入其他语言。

如果游戏没有官方英文名称，对于日文可以使用 `罗马音 / 原名`。比如 `バカ部` 可以将此字段设置为 `Bakabu / バカ部`。
...
此处应写英文描述。我们会在后面介绍国际化机制，在那里输入其他语言。
...
简化的描述应该更短，最小 200 个字符，最大 400 个字符。简化描述将用于在狭窄的地方和元数据中插入描述。

此处应写英文描述。我们会在后面介绍国际化机制，在那里输入其他语言。

这是一个可选的部分。对于不包含此部分的游戏，FGI 将使用标准描述进行自动裁剪。

简化的描述中允许使用换行符，但不支持 markdown 等富文本。
...
缩略图是游戏的品牌。请参考「[资源引用](#anchor_resref)」和「[缩略图资源文件规范](#anchor_thumbnail)」。
...
> FGI 曾经未制定缩略图的宽度、高度的标准。这影响了网站的视觉效果。对于新添加的游戏对应的缩略图、以及如果需要修改旧游戏的缩略图，则需要符合此规范。

缩略图的宽度为 360px，高度为 168px。文件大小在不造成明显的视觉差异时应小于 20KiB，但最大容忍 50KiB 的缩略图。

对于来自 Steam 的缩略图，已自动符合该比例，只需要按比例缩放到 360x168 px 即可。
对于来自其他地方的缩略图，通常情况下都将导致丢失信息。应尽量保证主要内容不被裁剪掉。

以下方式是不被允许的：

- 通过直接改变原始图片的比例来达到要求，从而避免裁剪。
- 通过添加白边或边框来使其比例达到要求，从而避免裁剪。
- 裁剪到文字，使部分或全部文字残缺。（要么保留完整的文字，要么将整块文字全部裁剪掉）

需要特别注明的几点：

- 可以通过图片编辑技术，改变图片中元素的相对位置。
- 带有透明通道的图片在没有“半透明”的情况下可以容忍，但如果透明区域只是充当边框的作用，没有任何其他元素时则不被允许。
...
> 值得一提的是，如果游戏没有该语言的官方名字翻译，你应该使用这种格式 `民间翻译 / 官方原名`，如 `漏夏 / 咱的夏天 / 漏れなつ`。
