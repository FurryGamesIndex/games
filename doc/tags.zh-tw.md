...
除非特殊說明，在 male 名稱空間表示獸人/人類生理性別是男性，在 female 名稱空間表示獸人/人類生理性別是女性
...
- `feline` 主要角色包括貓科獸人 (標籤別名：`felidae`)
- `canine` 主要角色包括犬科獸人 (標籤別名：`canidae`)
...
- `lung` 主要角色包括東方龍獸人 (標籤別名：`long`, `chinese-dragon`)
- `mouse` 主要角色包括鼠獸人
- `monkey` 主要角色包括猴子獸人
...
- `sheep` 主要角色包括羊獸人
- `horse` 主要角色包括馬獸人
- `rabbit` 主要角色包括兔獸人 (標籤別名：`hare`, `leporids`)
- `bear` 主要角色包括熊獸人
- `pig` 主要角色包括豬獸人
- `hyena` 主要角色包括鬣狗獸人
- `raccoon` 主要角色包括浣熊獸人
...
- `dog` 主要角色包括犬獸人 (標籤別名：`shibainu`, `husky`)
- `wolf` 主要角色包括狼獸人
- `fox` 主要角色包括狐獸人
...
- `transform` Main characters may transform (標籤別名：`transformation`)
- `transfur` 主要角色中可能出現獸化現象
- `anal` 主要角色之間包括肛交性行為
- `group` 主要角色之間包括群 P 行為
- `blowjob` 主要角色之間包括口交性行為 (標籤別名：`face-fucking`)
...
- `netorare` 另一個角色的配偶/伴侶與某角色發生關係。 (標籤別名：`ntr`)
- `rape` STUB
- `mind-break` 透過長時間或嚴格的性刺激來訓練/使某人在心理上成為性奴隸（洗腦）
- `mind-control` 強迫參與者（違背自己的意願）自己做某事（催眠） (標籤別名：`hypnosis`)
...
- `micro-macro` STUB (標籤別名：`macro-micro`)
- `mawplay` STUB
- `cock-worship` STUB
- `chastity-cage` STUB
- `belly-riding` STUB
...
- `work-in-process` 遊戲尚未完結（但有先行預覽版本或 Demo） (標籤別名：`ongoing`)
...
- `unofficial` 遊戲可能是玩家進行同人創作，如“漏夏（非公式版）”（不要和 misc:mods 混淆）
...
- `itchio` 遊戲在 itch.io 上有釋出 (標籤別名：`itch-io`, `itch`)
...
- `e-shop` 遊戲在 Nintendo E-Shop 上有釋出 (標籤別名：`nintendo-e-shop`)
- `gog` 遊戲在 gog.com 商店上有釋出 (標籤別名：`gog-com`)
...
- `macos` 遊戲官方支援 Apple macOS (標籤別名：`macintosh`, `osx`, `macosx`)
- `linux` 遊戲官方支援 GNU/Linux (標籤別名：`gnu-linux`)
