    ...
    def render(self):
        ...
        def _add_search_icon_link(m):
            ...
            if not c.endswith(":"):
                ...
                return (
                    f"`{c}` <a target='_blank' href='search.html?tagx?{c}'>{icon}</a>"
                )
            else:
                return f"`{c[:-1]}`"

        context = self.new_context()
        context["content"] = conv_doc_markdown(
            self.fctx,
            "search_help",
            self.language,
        ...
        with self.sm_openw(
            "sensitive.html",
            priority="0.2",
            lastmod_file=find_local_file(self.fctx.dir_templates, ["sensitive.html"]),
        ) as f:
        ...
        with self.sm_openw(
            "preferences.html",
            priority="0.2",
            lastmod_file=find_local_file(self.fctx.dir_templates, ["preferences.html"]),
        ) as f:
