...
parser.add_argument('--with-rss', default=False, action='store_true', help='generate RSS feeds (if --btime-file, you need provide the btime file can be generated by scripts.update-btime.py, or (not suggestion) use --mtime-has-fixed, which need to run scripts/fix-mtime.sh first. enable both of them will use btime only)')
