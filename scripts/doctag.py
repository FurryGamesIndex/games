...
def doctag_tag(name, data):
    ...
    if "implication" in data:
        implications = (
            uilang[lang]["implication"] + "`" + "`, `".join(data["implication"]) + "`"
        )
        print(f"    - {implications}")
    elif name in extra_tagdep:
        implications = (
            uilang[lang]["implication"] + "`" + "`, `".join(extra_tagdep[name]) + "`"
        )
        print(f"    - {implications}")


with open("tags.yaml") as f:
    tags = yaml.load(f.read())

extra_tagdep = dict()
with open("tag-dependencies.yaml") as f:
    tagdep2 = yaml.load(f.read())
    for _, v in tagdep2.items():
        for tag, i in v.items():
            extra_tagdep[tag] = i

doctag_funcs = {"@namespace": doctag_namespace, "@class": doctag_class}
